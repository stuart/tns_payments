...
class TNSPayments::ConnectionTest < MiniTest::Unit::TestCase
  ...
  def test_successful_process_acs_request_returns_authorized
    ...
    assert_equal 'AUTHORIZED', response.response['response']['3DSecure']['gatewayCode']
  end
...
private
  ...
  def stub_successful_process_acs_request transaction, pares
    stub_request(:post, /https:\/\/:#{@gateway.api_key}@secure\.ap\.tnspayments\.com\/api\/rest\/version\/4\/merchant\/#{@gateway.merchant_id}\/3DSecureId\/#{transaction.three_d_s_id}/).
    with(:body => JSON.generate({
      '3DSecure'     => {'paRes' => pares},
    ...
    to_return :status => 200, :headers => {}, :body => "{\"3DSecure\":{\"authenticationRedirect\":{\"customized\":{\"acsUrl\":\"https://secure.ap.tnspayments.com:443/acs/MastercardACS/4272b87b-2cc0-4232-a96e-e678ecbe7455\",\"paReq\":\"\"}},\"summaryStatus\":\"AUTHORIZED\",\"xid\":\"OddfZxGSfwm3EhyGzWx0JhPuWD4=\"},\"3DSecureId\":\"#{transaction.three_d_s_id}\",\"merchant\":\"#{@gateway.merchant_id}\",\"response\":{\"3DSecure\":{\"gatewayCode\":\"AUTHORIZED\"}}}"
  end
  
  def stub_failed_process_acs_request transaction, pares
    stub_request(:post, /https:\/\/:#{@gateway.api_key}@secure\.ap\.tnspayments\.com\/api\/rest\/version\/4\/merchant\/#{@gateway.merchant_id}\/3DSecureId\/#{transaction.three_d_s_id}/).
    with(:body => JSON.generate({
      '3DSecure'     => {'paRes' => pares},
      'apiOperation' => 'PROCESS_ACS_RESULT'
    ...
    to_return :status => 200, :headers => {}, :body => "{\"3DSecure\":{\"authenticationRedirect\":{\"customized\":{\"acsUrl\":\"https://secure.ap.tnspayments.com:443/acs/MastercardACS/4272b87b-2cc0-4232-a96e-e678ecbe7455\",\"paReq\":\"\"}},\"summaryStatus\":\"UNAUTHORIZED\",\"xid\":\"OddfZxGSfwm3EhyGzWx0JhPuWD4=\"},\"3DSecureId\":\"#{transaction.three_d_s_id}\",\"merchant\":\"#{@gateway.merchant_id}\",\"response\":{\"3DSecure\":{\"gatewayCode\":\"UNAUTHORIZED\"}}}"
  end
